...
# The crawl_web procedure takes a second parameter,
...
#limit the depth of the search on the seed page
def crawl_web(seed,max_depth):
    tocrawl = [seed]
    crawled = []
    next_depth = []
    depth = 0
    while tocrawl and depth <= max_depth:      #if list "tocrawled" is not empty and current depth is less than depth, you need to reach
        page = tocrawl.pop()
        if page not in crawled:
            union(next_depth, get_all_links(get_page(page)))   #get all the link pages that are in next level/depth
            crawled.append(page)
        if not tocrawl:      #if there is nothing left in tocrawl
            tocrawl, next_depth = next_depth, []     #tocrawl and next depth change values => tocrawl gets all pages that were in next depth and next depth is empty
            depth = depth + 1            #going on the next level
    return crawled
